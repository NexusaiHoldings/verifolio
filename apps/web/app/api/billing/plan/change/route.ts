/**
 * POST /api/billing/plan/change — apply a plan change (with proration).
 * Substrate shim for @nexus/billing-and-subscriptions (substrate-lego-wiring-001).
 */
import { NextResponse } from "next/server";
import { handleApplyPlanChange, type BillingConfig } from "@nexus/billing-and-subscriptions";
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  const result = await handleApplyPlanChange({
    userId: user?.id ?? null,
    body,
