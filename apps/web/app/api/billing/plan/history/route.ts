/**
 * GET /api/billing/plan/history — past plan changes for the current user.
 * Substrate shim for @nexus/billing-and-subscriptions (substrate-lego-wiring-001).
 */
import { NextResponse } from "next/server";
import { handlePlanHistory } from "@nexus/billing-and-subscriptions";
...
export async function GET(): Promise<NextResponse> {
  ...
  const result = await handlePlanHistory({
    userId: user?.id ?? null,
