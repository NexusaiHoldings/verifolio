"""Tests for the crm-and-lifecycle lego (crm-and-lifecycle-buildout-001).
Validates the real (non-stub) vertical slice: manifest, tools.yaml,
read-only vs side-effecting split, schema, and all-layers-present.
"""
...
class ManifestTests(unittest.TestCase):
    ...
    def test_not_a_stub(self) -> None:
        self.assertEqual(self.m["name"], "crm-and-lifecycle")
    ...
    def test_slots(self) -> None:
        ...
        self.assertEqual(names, {"lead_capture_form", "contact_sidebar", "after_lead_captured_hook"})

    def test_config_has_pipeline_and_outreach(self) -> None:
        props = self.m["config_schema"]["properties"]
        self.assertIn("pipeline", props)
        self.assertIn("outreach", props)
...
class ToolsYamlTests(unittest.TestCase):
    ...
    def test_five_tools(self) -> None:
        self.assertEqual(
            set(self.by_name),
            {"score_lead", "summarize_contact_history", "propose_next_action",
             "generate_outreach_draft", "advance_contact_stage"},
        )

    def test_read_only_tools(self) -> None:
        for name in ("score_lead", "summarize_contact_history", "propose_next_action",
                     "generate_outreach_draft"):
            self.assertFalse(self.by_name[name].get("side_effects"), f"{name} read-only")

    def test_advance_is_side_effecting_confirm(self) -> None:
        adv = self.by_name["advance_contact_stage"]
        self.assertTrue(adv["side_effects"])
        self.assertEqual(adv["action_class"], "confirm")
...
class SchemaTests(unittest.TestCase):
    def test_core_tables(self) -> None:
        ...
        for table in ("crm_contacts", "crm_interactions", "crm_outreach_drafts"):
...
class VerticalSliceTests(unittest.TestCase):
    def test_all_layers_present(self) -> None:
        for rel in (
            ...
            "api/contacts.ts", "api/outreach.ts", "api/_lib/handler.ts",
            "ui/components/LeadCaptureForm.tsx", "ui/components/ContactSidebar.tsx",
            "admin/routes.py", "events/crm-events.json", "docs/config.md",
            "docs/slots.md", "decisions/0001-crm-lego.md",
        ):
