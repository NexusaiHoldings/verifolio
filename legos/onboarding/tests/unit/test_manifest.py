"""Tests for the onboarding lego (onboarding-buildout-001)."""
...
class ManifestTests(unittest.TestCase):
    ...
    def test_not_a_stub(self) -> None:
        self.assertEqual(self.m["name"], "onboarding")
...
class ToolsYamlTests(unittest.TestCase):
    ...
    def test_four_tools(self) -> None:
        self.assertEqual(set(self.by_name),
                         {"recommend_onboarding_path", "detect_onboarding_dropoff",
                          "suggest_next_onboarding_step", "provision_sample_data"})

    def test_readonly_split(self) -> None:
        for n in ("recommend_onboarding_path", "detect_onboarding_dropoff", "suggest_next_onboarding_step"):
            self.assertFalse(self.by_name[n].get("side_effects"), f"{n} read-only")
        self.assertTrue(self.by_name["provision_sample_data"]["side_effects"])
        self.assertEqual(self.by_name["provision_sample_data"]["action_class"], "confirm")
...
class SchemaTests(unittest.TestCase):
    def test_core_tables(self) -> None:
        ...
        for table in ("onboarding_progress", "onboarding_sample_data_runs"):
...
class VerticalSliceTests(unittest.TestCase):
    def test_all_layers_present(self) -> None:
        for rel in (
            ...
            "api/onboarding.ts", "api/_lib/handler.ts", "ui/components/OnboardingChecklist.tsx",
            "admin/routes.py", "events/onboarding-events.json", "docs/config.md",
            "docs/slots.md", "decisions/0001-onboarding-lego.md",
        ):
