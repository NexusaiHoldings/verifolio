"""Tests for the developer-surface lego (developer-surface-buildout-001)."""
...
class ManifestTests(unittest.TestCase):
    ...
    def test_not_a_stub(self) -> None:
        self.assertEqual(self.m["name"], "developer-surface")
...
class ToolsYamlTests(unittest.TestCase):
    ...
    def test_four_tools(self) -> None:
        self.assertEqual(set(self.by_name),
                         {"summarize_api_usage", "detect_anomalous_api_traffic",
                          "recommend_rate_limit", "revoke_api_key"})

    def test_readonly_split(self) -> None:
        for n in ("summarize_api_usage", "detect_anomalous_api_traffic", "recommend_rate_limit"):
            self.assertFalse(self.by_name[n].get("side_effects"), f"{n} read-only")
        self.assertTrue(self.by_name["revoke_api_key"]["side_effects"])
        self.assertEqual(self.by_name["revoke_api_key"]["action_class"], "confirm")
...
class SchemaTests(unittest.TestCase):
    def test_core_tables(self) -> None:
        ...
        for table in ("api_keys", "webhooks", "api_request_log"):
...
class VerticalSliceTests(unittest.TestCase):
    def test_all_layers_present(self) -> None:
        for rel in (
            ...
            "api/devsurface.ts", "api/_lib/handler.ts", "ui/components/ApiKeysPanel.tsx",
            "admin/routes.py", "events/devsurface-events.json", "docs/config.md",
            "docs/slots.md", "decisions/0001-devsurface-lego.md",
        ):
